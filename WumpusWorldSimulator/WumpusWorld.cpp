...
WumpusWorld::WumpusWorld (int size)
{
	...
	initialMap = new MapState[size*size];
...
WumpusWorld::WumpusWorld (char* worldFile)
{
	...
	initialMap = new MapState[currentState.worldSize*currentState.worldSize];
	GenerateMap();
...
void WumpusWorld::GenerateMap()
{
	...
	for (y = currentState.worldSize; y > 0; y--)
	{
		...
		{
			initialMap[(y - 1)*currentState.worldSize + (x - 1)] = NONE;
			if (find(currentState.pitLocations.begin(), currentState.pitLocations.end(), Location(x, y)) != currentState.pitLocations.end())
			{
				...
				{
					if (currentState.goldLocation == Location(x, y))
						initialMap[(y - 1)*currentState.worldSize + (x - 1)] = PIT_WUMPUS_GOLD;
					else
						initialMap[(y - 1)*currentState.worldSize + (x - 1)] = PIT_WUMPUS;
				}
				else if (currentState.goldLocation == Location(x, y))
					initialMap[(y - 1)*currentState.worldSize + (x - 1)] = PIT_GOLD;
				else
					initialMap[(y - 1)*currentState.worldSize + (x - 1)] = PIT;
			}
			else if (currentState.wumpusLocation == Location(x, y))
			{
				if (currentState.goldLocation == Location(x, y))
					initialMap[(y - 1)*currentState.worldSize + (x - 1)] = WUMPUS_GOLD;
				else
					initialMap[(y - 1)*currentState.worldSize + (x - 1)] = WUMPUS;
			}
			else if (currentState.goldLocation == Location(x, y))
			{
				initialMap[(y - 1)*currentState.worldSize + (x - 1)] = GOLD;
			}
